...
def data_input():
    ...
    input_data['feature_three'] = pd.qcut(input_data['feature_three'], 5, labels=['v', 'w', 'x', 'y', 'z'])
    input_data['label'] = y
    return input_data
...
def test_train_model():
    """ Tests to make sure the model is a random classifer model; predict attribut is not none."""
...
def test_process_data_training_mode():
    ...
    X, y, encoder, lb = process_data(df, categorical_features=['feature_one'], label='label', training=True)
